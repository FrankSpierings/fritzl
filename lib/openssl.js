...
const Openssl = {
	...
	OPENSSL_sk_num: function(sk) {
		var name = "OPENSSL_sk_num";
		var address = Module.findExportByName(null, name);
		if (address) {
			var f = new NativeFunction(address, 'int', ['pointer']);
			var retval = f(sk);
	...
	OPENSSL_sk_value: function(sk, idx) {
		var name = "OPENSSL_sk_value";
		var address = Module.findExportByName(null, name);
		if (address) {
			var f = new NativeFunction(address, 'pointer', ['pointer', 'int']);
			var retval = f(sk, idx);
			return retval;
		} else { throw("Function '" + name + "' not found"); }
	},
	...
	PEM_write_bio_X509: function(bp, x) {
		var name = "PEM_write_bio_X509";
		var address = Module.findExportByName(null, name);
		if (address) {
			var f = new NativeFunction(address, 'int', ['pointer', 'pointer']);
			var retval = f(bp, x);
	...
	EVP_CIPHER_nid: function(e) {
		var name = "EVP_CIPHER_nid";
		var address = Module.findExportByName(null, name);
		if (address) {
			var f = new NativeFunction(address, 'int', ['pointer']);
			var retval = f(e);
