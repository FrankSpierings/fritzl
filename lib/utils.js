...
const Utils = {
	...
	hexdump: function(target, options) {
		...
		// Check if we received a ptr or a buffer
		var address = ptr(0);
		var buffer = null;
		if (target instanceof ArrayBuffer) {
			buffer = target.slice(0, options.length);
		} else {
			address = ptr(target);
			buffer = address.readByteArray(options.length);
		}
		...
		for (var i = 0; i < byteStrArray.length; i+=16) {
			...
			if(range) {
				addrStr = this.colorizeProtection(addrStr, range['protection']);
			}
